...
EcoTrace is a full-stack web application designed to gamify carbon footprint reduction. Built By PAL. By automating tracks via simulated integrations, calculating personal carbon matrices using local eco formulas, and applying behavioral science templates, it empowers users to sustain long-term eco-centric habits.
...
1. **2-Minute Carbon Footprint Quiz**
   - Estimates initial carbon profile baseline sectors (Home Energy, Travel, Food, Goods) using mathematically accurate default equations and static evaluations.
...
3. **Smart Integration Simulators**
   ...
   - **GPS Commute Tracker**: Integrates fitness steps to auto-log fuel-saving biking and walking.

4. **Local Analytics**
   - Analyzes user data through hard-coded local logics.
...
- **Backend / Deployment**: Served statically, fully local.
- **Styling**: Tailwind CSS (v4) with customized cubic-bezier transitions.

---

## 🔒 Build
Fully local codebase, requires no external secure API keys or secrets to operate.
